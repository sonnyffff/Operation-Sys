...
mkfs(void* image, size_t size, mkfs_opts* opts)
{
  ...
  int inode_block = opts->n_inodes/inodes_per_block + ((opts->n_inodes%inodes_per_block)==0?0:1);
  for(int i = 0;i < inode_block;i++) 
  {
    bitmap_set(dbmap, nblks, VSFS_ITBL_BLKNUM+i, true); // inode block
  }
  
  // TODO: Initialize the root directory.
  // 1. Mark root directory inode allocated in inode bitmap
   bitmap_set(ibmap,  opts->n_inodes,VSFS_ROOT_INO, true);   // root
  ...
  root_ino->i_blocks = 1;
  root_ino->i_direct[0] = VSFS_ITBL_BLKNUM + inode_block;
  root_ino->i_nlink = 2;
  ...
  bitmap_set(dbmap, nblks,root_ino->i_direct[0], true); // inode block
  ...
  int block_entries_num = VSFS_BLOCK_SIZE/sizeof(vsfs_dentry);
  for(int i = 2;i < block_entries_num;i++) {
  ...
  sb->data_region = VSFS_ITBL_BLKNUM + inode_block;
  sb->num_blocks = nblks;
  sb->free_blocks = nblks - sb->data_region - 1;
