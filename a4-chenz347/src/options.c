...
/**
 * CSC369 Assignment 4 - vsfs command line options parser implementation.
 */
...
#define VSFS_OPT(t, p)                                                         \
  {                                                                            \
    t, offsetof(vsfs_opts, p), 1                                               \
  }

static const struct fuse_opt opt_spec[] = { VSFS_OPT("-h", help),
                                            VSFS_OPT("--help", help),
                                            FUSE_OPT_END };
...
static int
opt_proc(void* data, const char* arg, int key, struct fuse_args* out)
{
